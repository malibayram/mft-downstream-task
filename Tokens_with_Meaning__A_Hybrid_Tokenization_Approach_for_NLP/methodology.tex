...
The proposed framework successfully integrates morphological analysis with subword segmentation, creating a robust tokenizer optimized for morphologically complex languages. By balancing linguistic integrity and computational efficiency, this approach offers a scalable solution adaptable to multiple languages.

To rigorously evaluate the impact of tokenization on downstream performance, we constructed a controlled experimental framework that isolates the tokenizer's effect from other architectural variables. We compared the proposed MFT tokenizer against three strong baselines representing different subword strategies: Tabi (a BERT-based Turkish tokenizer), CosmosGPT2 (a GPT-2 based tokenizer), and Mursit (a comprehensive tokenizer from NewMindAI). To ensure fair comparison, we initialized four identical embedding models based on the \texttt{google/embeddinggemma-300m} architecture. Key controls included random initialization, where all models were initialized with a fixed random seed (42) and no pretrained weights, ensuring that any performance difference is due solely to the inductive bias provided by the tokenizer's segmentation. Additionally, all models were configured with an identical vocabulary size of 32,768 tokens. We constructed a unified training dataset, \texttt{alibayram/cosmos-corpus-encoded}, containing pre-tokenized sequences for all four target tokenizers. While we focus on randomly initialized performance to test inductive bias, this unified corpus facilitates reproducible training and evaluation. Evaluation was performed on three standard benchmarks: Semantic Textual Similarity (STS) using the \texttt{figenfikri/stsb\_tr} dataset, the Massive Text Embedding Benchmark (MTEB-TR), and TurBLiMP for linguistic grammaticality testing. Future work may explore extending this framework to other agglutinative languages, optimizing it for real-time applications, and integrating additional linguistic features for enhanced performance.
