...
org_model_id = "magibu/embeddingmagibu-200m"
...
model.save_pretrained("magibu_cloned")

# override the model.safetensors file with the new embeddings
org_model.save_pretrained("magibu_cloned")
os.remove("magibu_cloned/tokenizer.json")
os.remove("magibu_cloned/tokenizer_config.json")

model = SentenceTransformer("magibu_cloned")
model = model.to(torch.bfloat16)

model.push_to_hub("alibayram/mft-downstream-task-embeddingmagibu", token=HF_TOKEN, exist_ok=True)

# remove the cloned folder
shutil.rmtree("magibu_cloned")
